...
namespace Memoria.Debugger
{
    public static class User32
    {
        ...
        public static Int32 IsTitleEquals(IntPtr windowHandle, params String[] titles)
        {
            StringBuilder sb = new StringBuilder(titles.Max(title => title.Length) + 2);
            Int32 size = GetWindowText(windowHandle, sb, sb.Capacity);

            for (var index = 0; index < titles.Length; index++)
            {
                var title = titles[index];
                if (title.Length == size && sb.ToString().StartsWith(title))
                    return index;
            }
